...
export default function TurnActivity({ activity }) {
  ...
  return <details className={`turn-activity turn-activity-${activity.status}`}>
    <summary>{activityLabel(activity)}{activity.usage?.total != null && <span> · {number(activity.usage.total)} tokens{!activity.usage.complete ? ' reported' : ''}</span>}</summary>
    <div className="turn-activity-body">
      ...
      <small>Counts update when the runtime reports usage; they are not a live token estimate or a context-window meter.</small>
